...
const ROLE_CONFIGS = {
  ...
  cliente: {
    ...
    quickReplies: [
      '📍 Buscar lavadores cerca',
      '⭐ ¿Cómo califico el servicio?',
      '❓ ¿Cómo funciona el pago digital?',
      '🚗 Tarifas de Car Wash'
    ],
    endpoint: '/ai/chat',
  ...
  fiscalizador: {
    ...
    quickReplies: [
      ...
      '📊 Reporte de formalización',
      '🔍 Buscar expediente de menor',
      '📋 Descargar planilla'
    ],
    endpoint: '/ai/chat',
  ...
  employer: {
    ...
    quickReplies: [
      '🔎 Buscar operarios de Car Wash',
      '✅ Ver candidatos verificados',
              ...
              <div className="flex gap-2 max-w-[85%]">
                ...
                <div
                  className={`rounded-2xl px-4.5 py-3 text-sm leading-relaxed shadow-md ${
                    isModel
                      ? 'text-[#F7FAFC] rounded-bl-none'
                      : 'text-white rounded-br-none font-medium'
                  }`}
                  style={{
                    backgroundColor: isModel ? config.burbujaBot : config.burbujaUser,
                    whiteSpace: 'pre-line'
                  }}
                >
                  {/* Si es Ramiro recomendando perfiles reales, podemos renderizarlo de forma amigable */}
                  {msg.text}
                </div>
              </div>
            </div>
      ...
      <div className="px-6 py-2 flex flex-wrap gap-2 border-t border-gray-800/30 bg-[#0c0d12]/50">
        {config.quickReplies.map((qr, idx) => (
          <button
            ...
            disabled={loading}
            className="text-left text-xs px-3.5 py-2 rounded-xl transition-all border border-gray-800 hover:bg-[#1a1c24] text-gray-300 hover:text-white disabled:opacity-40 select-none min-h-[44px] flex items-center font-medium"
            style={{
      ...
      <div className="p-6 border-t border-gray-800 bg-[#0c0d12]">
        <div className="flex gap-3">
          ...
          <button
            ...
            className="text-white px-6 py-3.5 rounded-2xl text-sm font-bold shadow-md transition-all flex items-center justify-center gap-1 min-w-[90px]"
            style={{
